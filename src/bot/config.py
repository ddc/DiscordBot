...
class Config(commands.Cog):
    ...
    async def config_pfilter(self, ctx, *, stats_channel: str):
        ...
        embed = discord.Embed(description=msg, color=color)
        profanityFilterSql = ProfanityFilterDal(self.bot.db_session, self.bot.log)
    ...
    async def config_config_list(self, ctx):
        ...
        sc = await serverConfigsSql.get_server_by_id(ctx.guild.id)
        ...
        embed.add_field(name="Text channels with profanity filter activated", value=chat_formatting.inline(channel_names),
                        inline=False)
