...
################################################################################
    @commands.group()
    @commands.cooldown(1, utils.get_settings("Cooldowns", "RollDiceCooldown"), BucketType.user)
    async def roll(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            if dice_size > 1:
                ...
                diceRollsSql = DiceRollsSql(self.bot.log)
...
################################################################################
    ...
    async def roll_results(self, ctx):       
        ...
        if member_str is not None:
            ...
            if len(rs) == 0:
                await utils.send_error_msg(self, ctx,f"There are no dice rolls of the size {dice_size} in this server.")
                return
...
################################################################################        
    @commands.command()
    @commands.cooldown(1, utils.get_settings("Cooldowns", "MiscCooldown"), BucketType.user)
...
################################################################################
    ...
    async def invites(self, ctx):
        ...
        if unlimited_invites:
            embed.add_field(name=f'Unlimited Invites ({len(unlimited_invites)})',value='\n'.join(unlimited_invites[:5]))
        if limited_invites:
