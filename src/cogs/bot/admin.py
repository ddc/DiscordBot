...
##############################################################################
    ...
    async def ban(self, ctx, member:discord.Member, *, reason=None):
        ...
        if member.id == self.bot.owner.id:
            ...
            return
        if member.id == self.bot.user.id:
            await utils.send_error_msg(self, ctx, "The Bot itself cannot be banned.")
            return
        if member.id == ctx.message.author.id:
            await utils.send_error_msg(self, ctx, "You cannot ban yourself.")
            return
        if utils.is_server_owner(ctx, member):
            await utils.send_error_msg(self, ctx, "You cannot ban the Server's Owner.")
            return
        if utils.is_member_admin(member):
            await utils.send_error_msg(self, ctx, "You cannot ban a Server's Admin.")
            return
        ...
        if len(banned_list) > 0:
            for banned_user in banned_list:
                if banned_user.user == user:
                    ...
                    await utils.send_msg(self, ctx, f"User `{user}` is no longer banned.")
                else:
    ...
    async def blacklist_list(self, ctx):
        ...
        if len(rs) > 0:
            for key, value in rs.items():
                ...
                author_name = utils.get_member_name_by_id(self, ctx, value["discord_author_id"])
    ...
    async def cc_add(self, ctx, command_name:str, *, text:str):
        ...
        for cmd in self.bot.commands:
            if str(command_name) == str(cmd.name).lower():
                await utils.send_error_msg(self, ctx, f"`{ctx.prefix}{command_name}` is already a standard command.")
                return
