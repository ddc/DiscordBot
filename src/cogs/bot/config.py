...
################################################################################ 
    ...
    @commands.cooldown(1, utils.get_settings("Cooldowns", "ConfigCooldown"), BucketType.user)
...
################################################################################ 
    ...
    async def config_pfilter(self, ctx, *, stats_channel:str):
        ...
        embed = discord.Embed(description=msg, color=color)
        profanityFilterSql = ProfanityFilterSql(self.bot.log)
...
################################################################################ 
    ...
    async def config_config_list(self, ctx):
        ...
        embed.add_field(name=f"Admins can be blacklisted (cannot use any commands)\n*`{ctx.prefix}config bladmin [on | off]`*", value=f"{on}" if sc[0]["blacklist_admins"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Admins can be muted (cannot type anything)\n*`{ctx.prefix}config muteadmin [on | off]`*", value=f"{on}" if sc[0]["mute_admins"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Display a message when someone joins the server\n*`{ctx.prefix}config joinmessage [on | off]`*", value=f"{on}" if sc[0]["msg_on_join"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Display a message when someone leaves the server\n*`{ctx.prefix}config leavemessage [on | off]`*", value=f"{on}" if sc[0]["msg_on_leave"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Display a message when the server gets updated\n*`{ctx.prefix}config servermessage [on | off]`*", value=f"{on}" if sc[0]["msg_on_server_update"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Display a message when someone changes their profile\n*`{ctx.prefix}config membermessage [on | off]`*", value=f"{on}" if sc[0]["msg_on_member_update"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Block messages from invisible members\n*`{ctx.prefix}config blockinvisible [on | off]`*", value=f"{on}" if sc[0]["block_invis_members"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Mention everyone when pool commands are used\n*`{ctx.prefix}config mentionpool [on | off]`*", value=f"{on}" if sc[0]["mention_everyone_pool_cmd"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Anonymous pools\n(hide the author's name from the pool command)\n*`{ctx.prefix}config anonymouspool [on | off]`*", value=f"{on}" if sc[0]["anonymous_pool"]=='Y' else f"{off}", inline=False)
        embed.add_field(name=f"Bot will react to member words\n*`{ctx.prefix}config botreactions [on | off] <channel_name>`*", value=f"{on}" if sc[0]["bot_word_reactions"]=='Y' else f"{off}", inline=False)
        ...
        embed.add_field(name=f"Text channel to display bot messages\n(defaults to the top first public channel)\n*`{ctx.prefix}config defaultchannel <channel_name>`*", value=formatting.inline(default_text_channel), inline=False)
        embed.add_field(name="Text channels with profanity filter activated", value=formatting.inline(channel_names), inline=False)
