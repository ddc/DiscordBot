...
from src.cogs.gw2.utils.gw2_exceptions import (APIBadRequest,APIConnectionError,APIError,APIForbidden,
        ...
        async with self.bot.aiosession.get(url, headers=headers) as response:
            if response.status != 200 and response.status != 206:
                ...
                if response.status == 400:
                    ...
                    raise APIBadRequest(self.bot, f"({response.status})({endpoint.split('?')[0]}) GW2 API is currently down. Try again later.")
                ...
                if response.status == 403:
                    if err_msg == "invalid key":
                        raise APIInvalidKey(self.bot, f"({response.status}) INVALID GW2 API key.")
                    raise APIForbidden(self.bot, f"({response.status})({endpoint.split('?')[0]}) Access denied with your GW2 API key.")
                
                if response.status == 502 or response.status == 503 or response.status == 504:
                    raise APIInactiveError(self.bot, f"({response.status})({endpoint.split('?')[0]}) GW2 API is currently down. Try again later.")
                
                if response.status == 429:
                    raise APIConnectionError(self.bot, f"({response.status})({endpoint.split('?')[0]}) GW2 API Requests limit has been saturated. Try again later.")
                
                else:
                    if len(err_msg) == 0:
                        err_msg = str(response.reason)
                    raise APIConnectionError(self.bot, f"GW2 API ERROR ({response.status})({endpoint.split('?')[0]})({err_msg})")
