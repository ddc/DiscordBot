...
################################################################################
    async def gw2_account(self, ctx):
        ...
        if len(rs) == 0:
            await ctx.message.channel.trigger_typing()
            await utils.send_error_msg(self, ctx,"You dont have an API key registered in this server.\n"\
            ...
            if "account" not in permissions:
                return await utils.send_private_error_msg(self, ctx,"Your API key doesnt have permission to access your gw2 account.\n"\
...
#                    embed.add_field(name="Achievements Points", value=formatting.inline(achiev_points))
                ...
                days = hours / 24
                #years = days / 365.25
                embed.add_field(name="Created", value=formatting.inline(f"{created} ({round(days)} days ago)"), inline=False)
