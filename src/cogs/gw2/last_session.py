...
############################################################################### 
    async def gw2_lastSession(self, ctx):
        ...
        if len(rs_api_key) == 0:
            return await utils.send_error_msg(self, ctx,"You dont have an API key registered in this server.\n"\
        ...
        if account == False or wallet == False or progression == False or characters == False:
            ...
            error_msg += "Please add another API key with permissions that are MISSING if you want to use this command.\n"\
                        ...
                        f"To check your API key use: `{ctx.prefix}gw2 key info`"
            return await utils.send_error_msg(self, ctx, error_msg)

        gw2LastSessionSql = Gw2LastSessionSql(self.bot.log)
        ...
        if len(rs_last_session) > 0:
            ...
            embed.set_author(name=f"{ctx.message.author.display_name}'s GW2 Last Session ({st_date})", icon_url=ctx.message.author.avatar_url)               
            ...
            if rs_last_session[0]["end_date"] is None:
                rs_last_session[0]["end_date"] = utils.get_todays_date_time()
                
            st_time = rs_last_session[0]["start_date"].split()[1]
            ed_time = rs_last_session[0]["end_date"].split()[1]
            #time_passed = utils.get_time_passed(str(ed_time))            
...
#                             if wait_time == "1":
#                                 m = "minute"
#                             return await utils.send_msg(self, ctx,f"{ctx.message.author.mention} Anet API still updating your stats!\n"\
#                                                  f"Please wait {wait_time} more {m} to try again.")                
            ...
            if updated_last_session[0]["start_yaks_scorted"] != updated_last_session[0]["end_yaks_scorted"]:
                final_result = str(updated_last_session[0]["end_yaks_scorted"] - updated_last_session[0]["start_yaks_scorted"])
            ...
            if updated_last_session[0]["start_wvw_tickets"] != updated_last_session[0]["end_wvw_tickets"]:
                final_result = str(updated_last_session[0]["end_wvw_tickets"] - updated_last_session[0]["start_wvw_tickets"])
                if int(final_result) > 0:
            ...
            if updated_last_session[0]["start_test_heroics"] != updated_last_session[0]["end_test_heroics"]:
                final_result = str(updated_last_session[0]["end_test_heroics"] - updated_last_session[0]["start_test_heroics"])
                if int(final_result) > 0:
                    embed.add_field(name="Gained test. heroics", value=formatting.inline(f"+{final_result}"), inline=True) 
                elif int(final_result) < 0:
            ...
            if updated_last_session[0]["start_proof_heroics"] != updated_last_session[0]["end_proof_heroics"]:
                final_result = str(updated_last_session[0]["end_proof_heroics"] - updated_last_session[0]["start_proof_heroics"])
                if int(final_result) > 0:
                    embed.add_field(name="Gained proof heroics", value=formatting.inline(f"+{final_result}"), inline=True) 
                elif int(final_result) < 0:
            ...
            if updated_last_session[0]["start_badges_honor"] != updated_last_session[0]["end_badges_honor"]:
                final_result = str(updated_last_session[0]["end_badges_honor"] - updated_last_session[0]["start_badges_honor"])
                if int(final_result) > 0:
                    embed.add_field(name="Gained badges of honor", value=formatting.inline(f"+{final_result}"), inline=True) 
                elif int(final_result) < 0:
                    embed.add_field(name="Lost badges of honor", value=formatting.inline(str(final_result)), inline=True)

            if updated_last_session[0]["start_guild_commendations"] != updated_last_session[0]["end_guild_commendations"]:
                final_result = str(updated_last_session[0]["end_guild_commendations"] - updated_last_session[0]["start_guild_commendations"])
                if int(final_result) > 0:
                    embed.add_field(name="Gained guild commendations", value=formatting.inline(f"+{final_result}"), inline=True) 
                elif int(final_result) < 0:
                    embed.add_field(name="Lost guild commendations", value=formatting.inline(str(final_result)), inline=True)
            ...
            await utils.send_error_msg(self, ctx,"No records were found in your name.\n"\
