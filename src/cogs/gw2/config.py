...
async def _list(self, ctx):
    ...
    embed.set_author(name=f"Guild Wars 2 configurations for {ctx.message.channel.guild.name}", icon_url=f"{ctx.message.channel.guild.icon_url}")
    embed.set_footer(text=f"For more info: {ctx.prefix}help gw2 config")
        
    gw2Configs = Gw2Configs(self.bot.log)
    rs = await gw2Configs.get_gw2_server_configs(ctx.message.channel.guild.id)
    
    if len(rs) == 0:
        last_session = "N"
        role_timer = f"{gw2Utils.get_settings('Defaults', 'CheckRoleTimer')} secs (Default)"
    else:
    ...
    embed.add_field(name="Bot should record gw2 users last sessions", value=f"{on}" if last_session=="Y" else f"{off}", inline=False)
    embed.add_field(name="Timer the bot should check for api roles in seconds", value=f"{formatting.box(role_timer)}", inline=False)
...
async def _apirole(self, ctx, stats_sever:str):
    ...
    if new_status == "Y":
        if server_has_role_already is None:
            new_role = await ctx.guild.create_role(name=new_server, permissions=discord.Permissions(permissions=116771904))
        else:
