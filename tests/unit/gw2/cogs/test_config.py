...
from src.gw2.cogs.config import GW2Config, GW2ConfigView, config, config_list, config_session
...
class TestGW2ConfigView:
    ...
    async def test_handle_update_exception_with_edit_failure(self, config_view, mock_ctx):
        ...
        interaction.edit_original_response = AsyncMock(side_effect=[None, Exception("Edit failed")])
    ...
    async def test_on_timeout_message_not_found(self, config_view):
        ...
        config_view.message.edit = AsyncMock(side_effect=discord.NotFound(mock_response, "Not found"))
    ...
    async def test_on_timeout_http_exception(self, config_view):
        ...
        mock_response = MagicMock()
        mock_response.status = 500
        config_view.message.edit = AsyncMock(side_effect=discord.HTTPException(mock_response, "Server error"))
