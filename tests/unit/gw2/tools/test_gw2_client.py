...
class TestHandleApiError:
    ...
    async def test_endpoint_split_on_question_mark(self, gw2_client):
        ...
        with pytest.raises(APIConnectionError) as exc_info:
            await gw2_client._handle_api_error(mock_response, "https://api.guildwars2.com/v2/achievements?ids=1,2,3")
...
class TestCallApiIntegration:
    ...
    @pytest.mark.asyncio
    async def test_call_api_non_200_206_returns_none_after_error_handler(self, gw2_client):
        """Test that call_api returns None after _handle_api_error for non-raising errors (line 40).

        When _handle_api_error does NOT raise (e.g., patched to do nothing),
        call_api should return None on line 40.
        """
        mock_response = AsyncMock()
        mock_response.status = 500
