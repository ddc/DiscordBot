...
class TestConfigPfilterChannelResolution:
    ...
    async def test_pfilter_no_channel_specified_uses_current(self, mock_send_embed, mock_dal_class, config_cog, mock_ctx):
        ...
        mock_dal.insert_profanity_filter_channel.assert_called_once_with(
            99999, 11111, "test-channel", 12345
        )
    ...
    @patch('src.bot.cogs.admin.config.ProfanityFilterDal')
    @patch('src.bot.cogs.admin.config.bot_utils.send_embed')
    async def test_pfilter_channel_found_by_numeric_id(self, mock_send_embed, mock_dal_class, config_cog, mock_ctx, mock_text_channel):
        ...
        mock_dal.insert_profanity_filter_channel.assert_called_once_with(
            99999, 22222, "general", 12345
        )

    @pytest.mark.asyncio
    @patch('src.bot.cogs.admin.config.ProfanityFilterDal')
    @patch('src.bot.cogs.admin.config.bot_utils.send_embed')
    async def test_pfilter_channel_found_by_name(self, mock_send_embed, mock_dal_class, config_cog, mock_ctx, mock_text_channel):
...
class TestConfigViewHandleUpdate:
    ...
    async def test_handle_update_wrong_user_ephemeral(self, config_view, mock_ctx):
        ...
        await config_view._handle_update(
            interaction, button, "msg_on_join",
            AsyncMock(), "Join Messages"
        )
    ...
    async def test_handle_update_toggles_from_false_to_true(self, mock_dal_class, config_view, mock_ctx, server_config):
        ...
        with patch.object(type(config_view), 'children', new_callable=PropertyMock, return_value=[mock_child]):
            await config_view._handle_update(
                interaction, button, "msg_on_leave",
                update_method, "Leave Messages"
            )
    ...
    @pytest.mark.asyncio
    @patch('src.bot.cogs.admin.config.ServersDal')
    async def test_handle_update_disables_buttons_during_processing(self, mock_dal_class, config_view, mock_ctx, server_config):
...
class TestConfigViewRestoreButtons:
    ...
    async def test_restore_buttons_sets_correct_styles(self, config_view, server_config):
        ...
        with patch.object(type(config_view), 'children', new_callable=PropertyMock, return_value=[mock_child1, mock_child2]):
